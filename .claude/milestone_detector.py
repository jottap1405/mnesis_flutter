...
class MilestoneDetector:
    ...
    def detect_milestone_mode(self) -> Optional[MilestoneData]:
        ...
        try:
            ...
            if not milestone_name:
                # No milestone context file found - try GitHub milestone
                github_milestone_name = self._check_github_milestone()
                if not github_milestone_name:
                    # No GitHub milestone either - normal mode
                    return None
                milestone_name = github_milestone_name
    ...
    def is_milestone_mode_active(self) -> bool:
        ...
        """
        try:
            # Check local milestone context file first
            if self._milestone_context_file.exists() and bool(
                self._read_milestone_context_file()
            ):
                return True

            # Check GitHub milestone as fallback
            return bool(self._check_github_milestone())
        except Exception:
            return False
    ...
    def _check_github_milestone(self) -> Optional[str]:
        """
        Check if current issue branch has a GitHub milestone.

        Returns:
            Optional[str]: GitHub milestone name if found, None otherwise
        """
        try:
            import subprocess
            import re
            ...
            for pattern in issue_patterns:
                ...
                if match:
                    ...
                    if candidate.isdigit() and 1 <= int(candidate) <= 999999:
                        issue_num = candidate
                        break
            ...
            ], capture_output=True, text=True, timeout=5, cwd=self.base_path)

            if gh_result.returncode != 0 or not gh_result.stdout.strip():
                return None

            milestone_name = gh_result.stdout.strip()
            if milestone_name and len(milestone_name) <= self.MAX_MILESTONE_NAME_LENGTH:
                return milestone_name

            return None

        except Exception:
            # Any error - return None for normal mode
            return None
